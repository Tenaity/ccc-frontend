...
export function useFixedOff(year: number, month: number) {
  ...
  const createFixed = async (payload: Omit<FixedAssignment, 'id'>) => {
  ...
  const updateFixed = async (id: number, payload: Partial<FixedAssignment>) => {
  ...
  const createOff = async (payload: Omit<OffDay, 'id'>) => {
  ...
  const deleteOff = async (id: number) => {
